...
declare -A MIN=( [schema_scenarios]=203 [rls_authorization]=611 )
...
# consistent; it does NOT prove that the hosted database, which is at 45, can
...
up_fail=0
for f in $(ls supabase/migrations/*.sql | head -45); do
...
if [ $up_fail -eq 0 ]; then
  line "production-equivalent base" "PASS (45 applied)"
  # The upgrade path from the CURRENT production migration count. 0045 is
  # deployed; 0046 is what this prompt adds.
  for f in supabase/migrations/0046_*.sql; do
