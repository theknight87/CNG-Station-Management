...
declare -A MIN=( [schema_scenarios]=166 [rls_authorization]=601 )
...
up_fail=0
for f in $(ls supabase/migrations/*.sql | head -44); do
...
if [ $up_fail -eq 0 ]; then
  line "production-equivalent base" "PASS (44 applied)"
  # The upgrade path from the CURRENT production migration count. 0044 is
  # deployed; 0045 is what this prompt adds.
  for f in supabase/migrations/0045_*.sql; do
